...
test('job HTTP mutations enforce owner, contributor and admin access for sessions and PATs', { timeout: 60_000 }, async (context) => {
  ...
  assert.equal((await call('/api/jobs/import', 'POST', credentials.Owner[0], { ...localOptions, zipName: 'deleted.zip' })).status, 404);
  const localImport = await call('/api/jobs/import', 'POST', mobileHeaders.Owner, localOptions);
